...
from ipad_agent.core.paths import CACHE_DIR, DEFAULT_CONFIG_PATH, RUNTIME_ROOT, descriptor_has_extended_acl
...
class Config:
    ...
    def browser_bundle(self) -> str:
        ...
        from ipad_agent.core.registry import IntegrationNotFoundError, load_registry
...
def _read_toml(path: Path, *, require_private: bool) -> dict[str, Any]:
    ...
    try:
        ...
        if require_private and descriptor_has_extended_acl(descriptor):
            raise ConfigError(f"config file must not carry an extended ACL: {path}")
...
def _enforce_addon_alias_gates(
    ...
    if not candidates:
        return
    from ipad_agent.core.registry import RegistryError, indexed_addon_for_bundle, normalize_name
