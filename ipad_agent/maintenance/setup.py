...
from ipad_agent.maintenance.bootstrap import APPIUM, bootstrap_automation
from ipad_agent.core.config import DEFAULT_CONFIG_PATH, load_config
from ipad_agent.maintenance.doctor import internal_report, public_report, public_text, run_doctor
from ipad_agent.core.paths import private_write_text
from ipad_agent.maintenance.versions import APPIUM_VERSION, SETUP_SCHEMA, XCUITEST_VERSION
...
def _setup_report(apply: bool, phase: str, changed: list[str], operations: list[dict[str, Any]], doctor: dict[str, Any], error: dict[str, str] | None) -> dict[str, Any]:
    ...
    operations_ok = all(item.get("ok") is not False for item in operations)
    ready = bool(doctor.get("ready")) and error is None and operations_ok
    ...
    if human_error:
        ...
        exit_code = max(20, int(doctor.get("exit_code", 20)))
    result: dict[str, Any] = {
