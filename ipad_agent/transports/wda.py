...
from ipad_agent.maintenance.bootstrap import APPIUM, APPIUM_HOME
from ipad_agent.core.paths import (
    RUNTIME_ROOT, private_mkdir, private_read_text, private_write_bytes,
    private_write_text, require_runtime_path,
)
from ipad_agent.maintenance.versions import (
...
APPIUM_START_MARKER = WDA_STATE_ROOT / "appium-starting.json"
...
def short_session(config: XCTestConfig, *, timeout: float = 180.0) -> Iterator[AppiumSession]:
    ...
    selected_device = _discover_ipad(min(timeout, 30.0), selected=config.udid)
    if selected_device.get("udid") != config.udid:
        raise XCTestControlError("WDA target is not one paired physical iPad")
    ensure_appium_server(config.appium_url)
    ownership = _prove_appium_endpoint(config.appium_url)
    capabilities = {
    ...
    server_pid: int | None = ownership[0]
    try:
...
def _stop_spawned_appium_without_receipt(
    process: subprocess.Popen[str], *, timeout: float,
) -> bool:
    """Stop the exact child handle when receipt creation fails."""
    try:
...
def ensure_appium_server(server_url: str = "http://127.0.0.1:4723", *, timeout: float = 8.0) -> None:
    ...
    ownership = _owned_appium_identity(server_url=server_url.rstrip("/"))
    if ownership is not None:
        if not _listener_owned_by_pid(port, ownership[0]):
            raise XCTestControlError(
                "Receipt-owned Appium does not own the configured listener",
                code="appium_endpoint_occupied",
            )
        try:
            _http_json("GET", f"{server_url.rstrip('/')}/status", None, 0.5)
        except XCTestControlError as error:
            raise XCTestControlError(
                "Receipt-owned Appium is not reachable; run owned cleanup before retrying",
                code="appium_owned_server_unreachable",
            ) from error
        return
    try:
        _http_json("GET", f"{server_url.rstrip('/')}/status", None, 0.5)
        ...
        raise XCTestControlError(
            "An unowned process already answers at the configured Appium endpoint",
            code="appium_endpoint_occupied",
        )
    local_appium = APPIUM
    ...
    private_write_text(
        APPIUM_START_MARKER,
        json.dumps({
            "schema": "ipad-agent.appium-starting/v1", "owner": OWNER,
            "nonce": nonce, "server_url": server_url.rstrip("/"),
            "executable": str(Path(appium).expanduser()), "command": command,
    ...
    try:
        private_write_text(
            APPIUM_OWNER_RECEIPT,
            json.dumps(receipt, sort_keys=True, separators=(",", ":")),
        )
        APPIUM_START_MARKER.unlink()
    except BaseException as error:
        stopped = _stop_spawned_appium_without_receipt(_SERVER, timeout=5.0)
        _SERVER = None
        ...
        if not stopped:
            raise XCTestControlError(
                "Appium started but ownership receipt failed and shutdown was not proven",
                code="wda_cleanup_incomplete",
            ) from error
        raise XCTestControlError(
            "Appium ownership receipt could not be written; spawned process was stopped",
            code="appium_start_failed",
        ) from error
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        ...
        try:
            _http_json("GET", f"{server_url.rstrip('/')}/status", None, 0.5)
            started_identity = _owned_appium_identity(server_url=server_url.rstrip("/"))
            if started_identity is None or not _listener_owned_by_pid(port, started_identity[0]):
                raise XCTestControlError(
                    "Started Appium listener ownership could not be proven",
                    code="appium_endpoint_occupied",
                )
            return
        except XCTestControlError:
...
def _http_json(method: str, url: str, payload: Any, timeout: float) -> dict[str, Any]:
    ...
    try:
        # Appium is a repository-owned loopback service. Never inherit HTTP
        # proxy settings for control traffic or device/session metadata.
...
def _discover_ipad(timeout: float, selected: str | None = None) -> dict[str, str]:
    ...
    for item in payload.get("result", {}).get("devices", []):
        ...
        if (
            ...
            and paired
            and all(isinstance(value, str) and value for value in (identifier, udid, version))
        ):
            matches.append({
...
def _privatize_xctestrun_alias_ancestors(
    selection: WDASelection, products: Path
) -> Path:
    """Narrow only Xcode's owned Build/Products directories for alias writes."""
    try:
        root = require_runtime_path(artifact_directory(selection))
        if root.name != selection.fingerprint:
            raise ValueError("artifact path does not match its fingerprint")
        build = require_runtime_path(root / "Build")
        expected_products = require_runtime_path(build / "Products")
        if products != expected_products:
            raise ValueError("Products path escapes the fingerprinted artifact")

        descriptors: list[tuple[Path, int]] = []
        flags = os.O_RDONLY | getattr(os, "O_DIRECTORY", 0)
        nofollow = getattr(os, "O_NOFOLLOW", 0)
        try:
            for directory in (build, expected_products):
                before = directory.lstat()
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise ValueError(f"expected a real directory: {directory}")
                if hasattr(os, "geteuid") and before.st_uid != os.geteuid():
                    raise PermissionError(
                        f"directory is not owned by the current user: {directory}"
                    )
                descriptor = os.open(directory, flags | nofollow)
                ...
                if (
                    ...
                    raise ValueError(f"directory changed during validation: {directory}")
            ...
            for directory, descriptor in descriptors:
                ...
                if stat.S_IMODE(narrowed.st_mode) != 0o700:
                    raise PermissionError(
                        f"directory mode could not be narrowed: {directory}"
                    )
        finally:
        ...
        raise XCTestControlError(
            f"WDA generated Build/Products ownership is unsafe: {error}",
            code="wda_setup_required",
        ) from error
...
def _materialize_xctestrun_alias(
    ...
    products = _privatize_xctestrun_alias_ancestors(selection, products)
...
def _config_with_runtime_compatibility(config: XCTestConfig) -> XCTestConfig:
    ...
    try:
        ...
        raise XCTestControlError(
            "WDA bootstrap path must belong to a project-owned verified artifact",
            code="wda_setup_required",
        ) from error
...
def validate_artifact(directory: str | os.PathLike[str], *, selection: WDASelection | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        payload = json.loads(private_read_text(root / ARTIFACT_FILE))
...
def verify_bounded_session(selection: WDASelection, *, apply: bool, timeout: float = 180.0, appium_url: str = "http://127.0.0.1:4723") -> dict[str, Any]:
    ...
    appium_stop = stop_owned_appium_server()
    if appium_stop.get("stopped") is not True:
        raise XCTestControlError(
            "Bounded WDA verification could not prove Appium shutdown",
            code="wda_cleanup_incomplete",
        )
...
def verified(selection: WDASelection) -> bool:
    try:
        payload = json.loads(private_read_text(verification_path(selection.fingerprint)))
        artifact = validate_artifact(artifact_directory(selection), selection=selection)
    ...
    return (
        ...
        and payload.get("appium_stopped") is True
        and teardown_proven
...
def _remove_appium_receipt_if_matches(pid: int, nonce: str) -> None:
    try:
        payload = json.loads(private_read_text(APPIUM_OWNER_RECEIPT))
...
def _owned_appium_identity(
    receipt: dict[str, Any] | None = None,
    *,
    server_url: str | None = None,
) -> tuple[int, str, str, bool] | None:
    """Return receipt-owned Appium identity only for the expected endpoint and PID."""
    if receipt is None:
        try:
            receipt = json.loads(private_read_text(APPIUM_OWNER_RECEIPT))
        except (OSError, json.JSONDecodeError):
            return None
    pid = receipt.get("pid")
    ...
    valid = (
        ...
        and (server_url is None or receipt.get("server_url") == server_url)
...
def _listener_owned_by_pid(port: int, pid: int) -> bool:
    """Prove that one exact PID is the sole TCP listener on the configured port."""
    ...
    if code != 0:
        return False
    pids = {
        int(line[1:]) for line in output.splitlines()
        if line.startswith("p") and line[1:].isdigit()
    }
    return pids == {pid}


def appium_start_marker_present() -> bool:
    """Return true unless absence of an unreceipted Appium start is proven."""
    try:
        private_read_text(APPIUM_START_MARKER)
    except FileNotFoundError:
        return False
...
def appium_endpoint_or_project_process_present(server_url: str) -> bool:
    """Detect any listener or repository-local Appium process when no receipt exists."""
    parsed = urlparse(server_url)
    if parsed.scheme != "http" or parsed.hostname not in {"127.0.0.1", "localhost", "::1"}:
        raise XCTestControlError("Appium URL must use loopback HTTP")
    port = parsed.port or 4723
    code, output, error = _run_process(
        ["/usr/sbin/lsof", "-nP", f"-iTCP:{port}", "-sTCP:LISTEN", "-Fp"],
        timeout=5,
    )
    if code == 0:
        pids = [line for line in output.splitlines() if line.startswith("p") and line[1:].isdigit()]
        if not pids:
            raise XCTestControlError("Appium listener inspection returned invalid data")
        return True
    if code != 1 or output or error:
        raise XCTestControlError("Appium listener absence could not be proven")
    code, output, error = _run_process(["/usr/bin/pgrep", "-f", str(APPIUM)], timeout=5)
    if code == 0:
        pids = [value for value in output.split() if value.isdigit()]
        if not pids:
            raise XCTestControlError("Appium process inspection returned invalid data")
        return True
    if code != 1 or output or error:
        raise XCTestControlError("Appium process absence could not be proven")
    return False


def _prove_appium_endpoint(server_url: str) -> tuple[int, str, str, bool]:
    parsed = urlparse(server_url)
    port = parsed.port or 4723
    ownership = _owned_appium_identity(server_url=server_url.rstrip("/"))
    if ownership is None or not _listener_owned_by_pid(port, ownership[0]):
        raise XCTestControlError(
            "Appium endpoint ownership could not be proven before session creation",
            code="appium_endpoint_occupied",
        )
    return ownership
...
__all__ = [
    ...
    "appium_endpoint_or_project_process_present", "appium_start_marker_present", "short_session", "stop_owned_appium_server", "validate_artifact", "verification_path", "verified",
