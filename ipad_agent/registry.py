...
def _validate_index(value: dict[str, Any], path: Path) -> list[dict[str, Any]]:
    ...
    for position, raw in enumerate(entries):
        ...
        expected_manifest = (
            f"{integration_id}/integration.json" if kind == "core"
            else f"../addons/{integration_id}/integration.json"
        )
        if manifest != expected_manifest:
            raise RegistryError(f"{entry_context}.manifest must be {expected_manifest!r}")
    ...
    indexed_core = {entry["id"] for entry in result if entry["kind"] == "core"}
    if indexed_core != CORE_INTEGRATIONS:
        missing = CORE_INTEGRATIONS - indexed_core
        extra = indexed_core - CORE_INTEGRATIONS
        details = []
        if missing:
            details.append(f"missing {', '.join(sorted(missing))}")
        if extra:
            details.append(f"unexpected {', '.join(sorted(extra))}")
        raise RegistryError("core integrations are fixed: " + "; ".join(details))
    return result


def _manifest_path(index_path: Path, entry: Mapping[str, Any]) -> Path:
    repository = index_path.parent.parent.resolve()
    expected_parent = repository / ("integrations" if entry["kind"] == "core" else "addons")
    candidate = (index_path.parent / entry["manifest"]).resolve()
    try:
        candidate.relative_to(expected_parent.resolve())
    except ValueError as error:
        raise RegistryError(
            f"{entry['id']} manifest must stay under {expected_parent}"
        ) from error
    return candidate
...
class Integration(Mapping[str, Any]):
    """A validated integration manifest with mapping and attribute access."""
...
class IntegrationRegistry:
    ...
    def __init__(self, index_path: str | Path = DEFAULT_INDEX_PATH, *, enabled_addons: Iterable[str] = ()) -> None:
        ...
        for entry in entries:
            ...
            integration = Integration(MappingProxyType(manifest), path, entry["category"])
