...
from .paths import REPO_ROOT, RUNTIME_ROOT, private_mkdir
...
def airdrop(path: str | os.PathLike[str]) -> dict[str, Any]:
    ...
    if status == "completed":
        return {"ok": True, "route": "airdrop", **result}
    if status == "uncertain":
        return {
            "ok": False,
            "route": "airdrop",
            **result,
            "error": str(result.get("reason") or "AirDrop outcome is unknown"),
            "uncertain": True,
        }
    return {
        "ok": False,
        "route": "airdrop",
        **result,
        "error": str(result.get("error") or "AirDrop failed"),
        "uncertain": False,
    }
