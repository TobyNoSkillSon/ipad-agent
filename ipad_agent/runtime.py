...
from .paths import private_write_text, require_runtime_path, runtime_path

DEFAULT_SOCKET_PATH = runtime_path("runtime.sock")
...
_REGISTRY_SNAPSHOT_SCHEMA = "ipad-agent.registry-snapshot/v1"
...
def _registry_contract(registry: Any) -> dict[str, Any]:
    ...
    for integration_id in sorted(registry.integrations):
        ...
        integrations.append({"category": integration.category, "manifest": integration.to_dict()})
    return {
...
def registry_snapshot(registry: Any) -> dict[str, Any]:
    """Serialize enough validated state to reconstruct an explicit registry."""
...
def registry_from_snapshot(value: Any) -> Any:
    ...
    expected = {"schema", "index_path", "enabled_addons", "contract_sha256"}
    ...
    actual = registry_snapshot(registry)["contract_sha256"]
...
def _socket_metadata_path(socket_path: Path) -> Path:
    path = require_runtime_path(socket_path)
...
class Runtime:
    ...
    def configure(self, config: Any, registry: Any = None) -> None:
        ...
        from .registry import IntegrationRegistry, load_registry
    ...
    def open_app(self, target: str, url: str | None = None):
        from .coredevice import open_ipad
    ...
    def selector(self, value: str) -> dict[str, Any]:
        ...
        from .registry import AddonNotEnabledError
    ...
    def screenshot(self, output: str | None = None) -> str:
        ...
        default = runtime_path("artifacts", "ipad-latest.png")
        target = Path(output).expanduser() if output else default
    ...
    def execute(self, request: dict[str, Any]) -> tuple[str, Any, bool]:
        ...
        if op == "o":
            ...
            launch = self.open_app(target, url)
...
class Daemon:
    def __init__(self, socket_path: Path, *, idle_ttl: float, nonce: str | None = None) -> None:
        self.socket_path = require_runtime_path(socket_path)
    ...
    def _bind(self) -> socket.socket:
        ...
        if self.socket_path.exists():
            ...
            with contextlib.suppress(FileNotFoundError):
                _socket_metadata_path(self.socket_path).unlink()
        ...
        try:
            ...
            self._socket_inode = socket_info.st_ino
            owner = {
...
def _client_start_daemon() -> bool:
    ...
    command = [
        sys.executable, "-m", "ipad_agent.daemon", "--socket", str(path),
        "--nonce", nonce,
    ]
    try:
        subprocess.Popen(
...
def run_daemon(socket_path: Path, idle_ttl: float, *, nonce: str | None = None) -> int:
    daemon = Daemon(require_runtime_path(socket_path), idle_ttl=idle_ttl, nonce=nonce)
