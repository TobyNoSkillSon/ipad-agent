...
from .bootstrap import APPIUM, APPIUM_HOME, node_remediation, resolve_node_toolchain
from .config import Config, load_config
from .versions import APPIUM_VERSION, DOCTOR_SCHEMA, XCUITEST_VERSION, version_tuple
...
def _automation_checks(checks: list[Check], node_ok: bool) -> tuple[bool, bool]:
    if not node_ok:
        checks.append(_skipped("automation.appium", "host", "Compatible Node.js/npm is unavailable")); checks.append(_skipped("automation.xcuitest_driver", "host", "Pinned Appium is unavailable"))
        return False, False
    env = dict(os.environ); env["APPIUM_HOME"] = str(APPIUM_HOME)
    probe = _run([str(APPIUM), "--version"], env=env)
    version = probe[1].splitlines()[0] if probe[0] == 0 and probe[1] else None
    appium_ok = version == APPIUM_VERSION
    checks.append(Check("automation.appium", "pass" if appium_ok else "fail", f"Appium {version}" if version else "Repository-local Appium is unavailable", "host", {"path": str(APPIUM), "version": version}, remediation=None if appium_ok else "Run: python3 -m ipad_agent setup --phase host --apply --json"))
    if not appium_ok:
        checks.append(_skipped("automation.xcuitest_driver", "host", "Pinned Appium is unavailable")); return False, False
    driver = _run([str(APPIUM), "driver", "list", "--installed", "--json"], env=env)
    version = None
    if driver[0] == 0:
        try:
            item = json.loads(driver[1]).get("xcuitest", {})
            version = item.get("version") if isinstance(item, dict) else None
        except (json.JSONDecodeError, AttributeError):
            pass
    ok = version == XCUITEST_VERSION
    checks.append(Check("automation.xcuitest_driver", "pass" if ok else "fail", f"XCUITest driver {version}" if version else "Repository-local XCUITest driver is unavailable", "host", {"version": version, "appium_home": str(APPIUM_HOME)}, remediation=None if ok else "Run: python3 -m ipad_agent setup --phase host --apply --json"))
    return appium_ok, ok
...
def _report(checks: list[Check], selected: dict[str, Any]) -> dict[str, Any]:
    # AirDrop readiness is capability-scoped: it must not make CoreDevice app
    # launches look unavailable. Its failed checks still carry agent remediation.
    blocking = [
        item for item in checks
        if item.status in {"fail", "action_required", "unknown"}
        and item.id not in _OPTIONAL_TRANSFER_CHECKS
    ]
...
def _node_supported(version: tuple[int, int, int]) -> bool:
    from .versions import node_supported
