"""Semantic display router for the paired iPad.
...
SERVER_MODULE = "ipad_agent.server"
...
def _browser_identity(config: "Config", registry: "IntegrationRegistry") -> tuple[str, str]:
    from .registry import AddonNotEnabledError, IntegrationNotFoundError
...
def _copy_asset(path: Path, meta: dict[str, Any]) -> tuple[str, str]:
    ...
    assets = RUNTIME_DIR / "assets"; assets.mkdir(exist_ok=True, mode=0o700)
    target = assets / name
    if not target.exists() or target.stat().st_size != path.stat().st_size:
        temp = target.with_suffix(target.suffix + ".tmp")
        shutil.copyfile(path, temp); os.chmod(temp, 0o600); temp.replace(target)
...
def show(op: object, *args: object, title: str | None = None, source: str | None = None,
    ...
    if inferred == "u":
        ...
        if urlparse(url).scheme not in {"http", "https"}: raise ValueError("show u requires an http(s) URL")
        return _direct(str(options.get("app") or browser), url, device=device, config=config, registry=registry)
    if inferred == "a":
    ...
    if inferred == "m":
        if not values: raise ValueError("show m requires a destination")
        destination = str(values[0]); origin = options.get("origin"); directions = bool(options.get("directions", True)); mode = str(options.get("mode") or "")
        params: dict[str, str] = {"daddr" if directions else "q": destination}
        if origin: params["saddr"] = str(origin)
        flags = {"driving": "d", "walking": "w", "transit": "r"}
        if mode in flags: params["dirflg"] = flags[mode]
        return _direct("maps", "maps://?" + urlencode(params), device=device, config=config, registry=registry)
    if inferred == "v":
