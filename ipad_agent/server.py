...
def atomic_json(path:Path,value:object)->None:
    temp=path.with_suffix(".tmp");temp.write_text(json.dumps(value,separators=(",",":")));os.chmod(temp,0o600);temp.replace(path)
...
def main()->int:
    ...
    BIND_HOST=args.host;RUNTIME_DIR=require_runtime_path(args.runtime_dir);RUNTIME_DIR.mkdir(parents=True,exist_ok=True,mode=0o700);os.chmod(RUNTIME_DIR,0o700);ASSET_DIR=RUNTIME_DIR/"assets";ASSET_DIR.mkdir(exist_ok=True,mode=0o700);os.chmod(ASSET_DIR,0o700)
    ...
    try:
        ...
        TOKEN=""
    if len(TOKEN)<24:
        TOKEN=secrets.token_urlsafe(24);token_path.write_text(TOKEN)
