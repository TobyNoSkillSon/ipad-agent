...
class RepositoryLayoutTests(unittest.TestCase):
    def test_package_is_repository_native(self):
        ...
        self.assertEqual(root / "ipad_agent", Path(runtime.__file__).resolve().parent)
    ...
    def test_daemon_starts_from_repository_root(self):
        ...
        self.assertEqual([runtime.sys.executable, "-m", "ipad_agent.daemon"], command[:3])
        self.assertEqual(str(root), popen.call_args.kwargs["cwd"])

    def test_display_server_uses_repository_module(self):
        root = Path(__file__).resolve().parents[1]
        self.assertEqual("ipad_agent.server", display.SERVER_MODULE)
