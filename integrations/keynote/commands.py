"""CoreDevice/AirDrop Keynote commands with app-owned file authority."""
...
def _load_authority() -> dict[str, dict[str, object]]:
    ...
    if data.get("schema") != "ipad-agent.keynote-file-authority/v1" or tuple(data.get("extensions", ())) != _EXTENSIONS:
        raise ValueError("Keynote file authority is invalid")
    ...
    if not isinstance(routes, list) or [item.get("command") for item in routes if isinstance(item, dict)] != list(COMMANDS):
        raise ValueError("Keynote compatibility authority is invalid")
    return {str(item["command"]): item for item in routes}

def _validated_file(value: object) -> object:
    _load_authority()
    try: text = os.fspath(value)
    except TypeError as error: raise ValueError("Keynote file must be one local path") from error
    if not isinstance(text, str) or not text or "\x00" in text:
        raise ValueError("Keynote file must be one non-empty local path")
    if not any(Path(text).name.casefold().endswith(extension) for extension in _EXTENSIONS):
        raise ValueError("Keynote file extension is not admitted")
    return value

def keynote(command: object, *args: object, **options: object) -> Any:
    try:
        ...
        if operation == "open":
            if route.get("availability") != "proven" or route.get("production") != "admitted": raise ValueError("Keynote open is candidate-gated on this profile")
            if args or options: raise ValueError("open takes no arguments")
            return shared._direct_open("Keynote")
        if len(args) != 1 or options: raise ValueError(f"{operation} requires exactly one local file path")
        path = _validated_file(args[0])
        return shared._airdrop(path) if operation == "drop" else shared._show_local("Keynote", path)
