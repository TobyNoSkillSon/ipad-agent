...
from integrations.pages import commands

DIRECTORY = Path(__file__).resolve().parents[1]

class ContractTests(unittest.TestCase):
    def test_wrapper(self): assert_public_wrapper_forwarding(self, public_name="ipadpages", package="pages", implementation="pages")
    def test_surface(self): self.assertEqual(commands.COMMANDS, ("open", "drop", "show"))
    def test_file_gate(self):
        for value in ('/tmp/file.PAGES', '/tmp/file.DOC', '/tmp/file.DOCX', '/tmp/file.RTF', '/tmp/file.TXT'): self.assertIs(commands._validated_file(value), value)
    ...
    def test_open_dispatches_after_exact_profile_proof(self):
        with mock.patch.object(commands.shared, "_direct_open", return_value="accepted") as direct: result=commands.pages("open")
        self.assertEqual(result, "accepted"); direct.assert_called_once_with("Pages")
    def test_file_dispatch_once(self):
        with mock.patch.object(commands.shared, "_airdrop", return_value="sent") as drop: self.assertEqual(commands.pages("drop", "/tmp/file.pages"), "sent")
