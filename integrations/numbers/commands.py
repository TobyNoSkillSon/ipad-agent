"""CoreDevice/AirDrop Numbers commands with app-owned file authority."""
...
_EXTENSIONS = ('.numbers', '.xls', '.xlsx', '.csv', '.tsv')
...
def _load_authority() -> dict[str, dict[str, object]]:
    ...
    if data.get("schema") != "ipad-agent.numbers-file-authority/v1" or tuple(data.get("extensions", ())) != _EXTENSIONS:
        raise ValueError("Numbers file authority is invalid")
    ...
    if not isinstance(routes, list) or [item.get("command") for item in routes if isinstance(item, dict)] != list(COMMANDS):
        raise ValueError("Numbers compatibility authority is invalid")
    return {str(item["command"]): item for item in routes}

def _validated_file(value: object) -> object:
    _load_authority()
    try: text = os.fspath(value)
    except TypeError as error: raise ValueError("Numbers file must be one local path") from error
    if not isinstance(text, str) or not text or "\x00" in text:
        raise ValueError("Numbers file must be one non-empty local path")
    if not any(Path(text).name.casefold().endswith(extension) for extension in _EXTENSIONS):
        raise ValueError("Numbers file extension is not admitted")
    return value

def numbers(command: object, *args: object, **options: object) -> Any:
    try:
        ...
        if operation == "open":
            if route.get("availability") != "proven" or route.get("production") != "admitted": raise ValueError("Numbers open is candidate-gated on this profile")
            if args or options: raise ValueError("open takes no arguments")
            return shared._direct_open("Numbers")
        if len(args) != 1 or options: raise ValueError(f"{operation} requires exactly one local file path")
        path = _validated_file(args[0])
        return shared._airdrop(path) if operation == "drop" else shared._show_local("Numbers", path)
