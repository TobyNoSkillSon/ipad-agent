...
from integrations.numbers import commands

DIRECTORY = Path(__file__).resolve().parents[1]

class ContractTests(unittest.TestCase):
    def test_wrapper(self): assert_public_wrapper_forwarding(self, public_name="ipadnumbers", package="numbers", implementation="numbers")
    def test_surface(self): self.assertEqual(commands.COMMANDS, ("open", "drop", "show"))
    def test_file_gate(self):
        for value in ('/tmp/file.NUMBERS', '/tmp/file.XLS', '/tmp/file.XLSX', '/tmp/file.CSV', '/tmp/file.TSV'): self.assertIs(commands._validated_file(value), value)
    ...
    def test_open_dispatches_after_exact_profile_proof(self):
        with mock.patch.object(commands.shared, "_direct_open", return_value="accepted") as direct: result=commands.numbers("open")
        self.assertEqual(result, "accepted"); direct.assert_called_once_with("Numbers")
    def test_file_dispatch_once(self):
        with mock.patch.object(commands.shared, "_airdrop", return_value="sent") as drop: self.assertEqual(commands.numbers("drop", "/tmp/file.numbers"), "sent")
